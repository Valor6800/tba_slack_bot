...
(function() {
    ...
    class TBA {
        ...
        httpGet(url, callback)
        {
            var options = {
                method: 'GET',
                url: config.tba_base_url + url,
                headers: {
                    'X-TBA-Auth-Key': config.tba_auth_token
                }
            };
        ...
        parseMatch(complete_match_data)
        {
            ...
            for (var color in colors) {
                ...
                for (var i = 0; i < 3; i++) {
                    ...
                    if (!(robot in results))
                        results[robot] = {};
                    for (var j = 0; j < this.component_opr_2019_keys.length; j++) {
                        results[robot][this.component_opr_2019_keys[j]] = sums[this.component_opr_2019_keys[j]];
                    }
        ...
        genOPRs(eventCode, callback)
        {
            this.getMatches(eventCode, function(matches) {
                    ...
                    if (matches[match].actual_time && (
                        ...
                        for (var team in parsed_match) {
                            ...
                            if (!(team in results)) {
                                ...
                                for (var i = 0; i < this.component_opr_2019_keys.length; i++) {
                                    results[team][this.component_opr_2019_keys[i]] = 0;
                                }
                            }
                            ...
                            for (var i = 0; i < this.component_opr_2019_keys.length; i++) {
                                let key = this.component_opr_2019_keys[i];
                ...
                for (var i = 0; i < this.component_opr_2019_keys.length; i++) {
                    B[this.component_opr_2019_keys[i]] = [];
                }
                ...
                for (var team in results) {
                    ...
                    //   [[245] [328]]
                    for (var i = 0; i < this.component_opr_2019_keys.length; i++) {
                        B[this.component_opr_2019_keys[i]].push(results[team][this.component_opr_2019_keys[i]]);
                    }
                }
                ...
                var opr_arrays = {};
                for (var i = 0; i < this.component_opr_2019_keys.length; i++) {
                    opr_arrays[this.component_opr_2019_keys[i]] = math.lusolve(A, B[this.component_opr_2019_keys[i]]);
                }
                ...
                for (var team in results) {
                    oprs[team] = {'opr': 0};
                    for (var j = 0; j < this.component_opr_2019_keys.length; j++) {
                        oprs[team][this.component_opr_2019_keys[j]] = opr_arrays[this.component_opr_2019_keys[j]][i][0];
                        oprs[team].opr += oprs[team][this.component_opr_2019_keys[j]];
                    }
                    i++;
                }
        ...
        // Internal helper functions (year specific)
        ...
        parse2019Match(match)
        {
            let sums = {};
            for (var i = 0; i < this.component_opr_2019_keys.length; i++) {
                sums[this.component_opr_2019_keys[i]] = 0;
            }
            ...
            for (var i = 1; i <= 8; i++) {
                ...
                if (scored_bay.includes('Cargo')) {
                    index = (i == 4 || i == 5) ? 8 : 9;
                    sums[this.component_opr_2019_keys[index]]++;
                }
            ...
            for (var i = 1; i <= 3; i++) {
                ...
                sums[this.component_opr_2019_keys[11]] += ((robot_climb == 3) ? 12 :
                                                      (robot_climb == 2) ? 6 :
                                                      (robot_climb == 1) ? 3 : 0);
                if (robot_auto_move)
                    sums[this.component_opr_2019_keys[10]] += ((robot_start == 2) ? 6 : 3);
            }

            // Foul points
            sums[this.component_opr_2019_keys[12]] += match['foulPoints'];

            // Convert scored elements into points
            for (var i = 0; i < 5; i++) {
                sums[this.component_opr_2019_keys[i]] *= 2;
            }
            for (var i = 5; i < 10; i++) {
                sums[this.component_opr_2019_keys[i]] *= 3;
            }
